...
echo "first allotment for religions<hr>";
		global $db;
$count = $db->query('SELECT COUNT(user_id) FROM applications',TRUE,TRUE);
if($count < 1){
	return;
}
...
echo " applicants in religion id ".$religion['religion_id']." <hr>";
echo nl2br(str_replace(' ','&nbsp;&nbsp;&nbsp;',print_r($applicants,TRUE))).'<hr>';
...
private function allot_applicant_to_courses($applicant){
	...
	while($loop){
		...
		// if($counter < 105)
		// 	echo "counter = ".$counter++."<br> current_preference".$this->applicant['current_preference']." <br>";
		// else
		// 	exit();
	...
	//Get preffered course from intilized courses.
		...
		if( ($this->applicant['alloted']==TRUE) OR (++$this->applicant['current_preference'] > 3) ){
			echo "<br>applicant['alloted'] : ".$this->applicant['alloted']."<br> current_preference : ".$this->applicant['current_preference']."<br>";
			$loop = FALSE;
		}
	}
...
private function  allot_applicant_to_course($prefered_course){
	...
	$this->courses = $temp;
	echo nl2br(str_replace(' ','&nbsp;&nbsp;&nbsp;',print_r($this->applicant,TRUE))).'<br>applicant <hr>';
	echo nl2br(str_replace(' ','&nbsp;&nbsp;&nbsp;',print_r($prefered_course,TRUE))).'<br>courses <hr>';
}
private function make_first_allotment_for_general_category($religions,$courses){
global $db;

	echo "first allotment for general category<hr>";
...
echo "general category applicants <hr>";
echo nl2br(str_replace(' ','&nbsp;&nbsp;&nbsp;',print_r($general_category_applicants,TRUE))).'<hr>';
...
echo "general category course seats<hr>";
echo nl2br(str_replace(' ','&nbsp;&nbsp;&nbsp;',print_r($courses_with_general_seats,TRUE))).'<hr>';
$courses_with_alloted_applicants_temp = $this->allot_applicants($general_category_applicants ,$courses_with_general_seats);
//add alloted students to database.
	$this->add_alloted_students_to_db($courses_with_alloted_applicants_temp);
	$db->query('UPDATE applications SET  current_preference = 1 ');
...
echo "before add to db <hr>";
echo nl2br(str_replace(' ','&nbsp;&nbsp;&nbsp;',print_r($this->courses,TRUE))).'<hr>';
...
public function make_next_allotment(){
	echo "next allotment for reigions<hr>";
	global $db;
	//get all relgions with reservation percentage > 0 and religions which has atleast one applicant.
		$religions = $db->query('SELECT 
			DISTINCT religions.religion_id, religions.reservation_percentage
...
$courses_with_alloted_applicants = $this->allot_applicants($applicants ,$courses_with_reserved_seats);
$this->add_alloted_students_to_db($courses_with_alloted_applicants);
		}//#foreach ($religions as $religion) 
$db->query('UPDATE applications SET  current_preference = 1 ');
echo "next allotments for general category<hr>";
